...
exports.getAllwithPanutan = async (req,res,next) => {
  ...
  console.log(daripanutan.data.data);
  ...
  panutan.map((pn) => { 
    ...
    keteranganstatus = keterangan_panutan;statusexp = 0;}
    arrsurat.push({
...
exports.create = async (req, res, next) => {
  ...
  let cekstatus = await SuratTugasPerjadin.findAll({
    where: {   id_surat_tugas: req.body.id_surat_tugas,kode_status:{[Op.in]:[3,4,5,6,7,8,9,10]} }
  });
  ...
  const count = countSurat[0].maks + 1;
    ...
    await SuratTugasPerjadin.create({
      id_surat_tugas: req.body.id_surat_tugas,
...
exports.updatestatus = async (req, res, next) => {
  try {
    ...
    await SuratTugasPerjadin.update({kode_status:req.body.kode_status}, {
      where: {
        id_surat_tugas: req.params.id_surat_tugas,kode_unit:req.params.kode_unit,tahun:req.params.tahun
      },
    });
...
exports.createNew = async (req, res, next) => {
  ...
  try{
    ...
    const update = await SuratTugasPerjadin.update({kode_rka:req.body.kode_rka,kode_periode:req.body.kode_periode},{where:{id_surat_tugas:req.body.id_surat_tugas}})
    .then((up)=>{
    ...
    const create = await db.transaction().then((t)=>{
      SuratTugasPerjadin.create({
       ...
       return PetugasPerjadinBiaya.bulkCreate(pegawai,{transaction:t}).then((pegInsert)=>{
        t.commit();
        axios .post(`${hostProdevPanutannew}${idAPI.panutan.update_status_perjadin}/${req.body.kode_unit}/${req.body.id_surat_tugas}`)
        return jsonFormat(res,"success","Berhasil menginputkan data",pegInsert);
       })
       .catch((err)=>{
        t.rollback()
        return next(err)})
